/*=============================================================================*
 *
 *  PRODUCT  : Interrupt Service Routine 
 *
 *  FILENAME : SC_DefaultIsr_BIOS.c 
...
//	GpioDataRegs.GPBTOGGLE.bit.GPIO40 = 1;	// light out the hardware watchdog

	PieCtrlRegs.PIEACK.all = PIEACK_GROUP1;	 // Must acknowledge the PIE group
}
//---------------------------------------------------------------------
// On the positive jump of Grid-R's voltage, the hardware produces a falling edge to ECAP5.
void ECAP5_INT_ISR(void)			// PIE4.5 @ 0x000D78  ECAP5_INT (ECAP5)
{
	...
	// Calculate Grid R frequency
	// Anti-Islanding
//	g_u32VgridRFreqCnt = ECap5Regs.CAP1;

	// Check grid phase sequence.

	ECap5Regs.ECCLR.bit.INT   = 1;			// Clear the ECAP5 interrupt flag
	ECap5Regs.ECCLR.bit.CEVT1 = 1;			// Clear the CEVT1 flag	
}

//---------------------------------------------------------------------
void ECAP6_INT_ISR(void)			// PIE4.6 @ 0x000D7A  ECAP6_INT (ECAP6)
{
	// Calculate Grid S frequency

//	g_u32VgridSFreqCnt = ECap6Regs.CAP1;

	ECap6Regs.ECCLR.bit.INT   = 1;			// Clear the ECAP6 interrupt flag
	ECap6Regs.ECCLR.bit.CEVT1 = 1;			// Clear the CEVT1 flag	
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP4;	// Must acknowledge the PIE group
}
...
void SCIRXINTA_ISR(void)			// PIE9.1 @ 0x000DC0  SCIRXINTA (SCI-A)
{
	...
	QueDataIn(g_pSciIndex[ID_SCI_IPOMS]->pqRx,u8Tmp);
#endif
...
void SCIRXINTB_ISR(void)			// PIE9.3 @ 0x000DC4  SCIRXINTB (SCI-B)
{
	...
	temp = (Uint8)(ScibRegs.SCIRXBUF.all & 0x00FF);
	switch(SciReceivePonit)
	{
		...
		default:
			if(SciReceivePonit == (SciRxBuff[1]+3))//Rev full 
			{
				...
				{
					SEM_post(&SEM_TskSCIbUpdateReady);
					SciReceivePonit = 0;
				}
				else
				{
...
/*============================================================================*/
//--- The following HWIs are not used in  project
/*============================================================================*/ 
