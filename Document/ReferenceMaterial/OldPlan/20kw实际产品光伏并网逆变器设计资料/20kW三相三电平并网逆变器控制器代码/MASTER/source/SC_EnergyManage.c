...
void PFLimitInit(void)
{
  //	g_GridManager.PFreq.u16Enable = 1;
	...
	g_GridManager.PFreq.f32FOback = 50.2;
...
//	g_SaftyConfig.bit.ReactiveMode = 1;
...
//			}
			g_GridManager.PFreq.u16FPLimit =  g_GridManager.PFreq.u16PowerM;//4105
			g_GridManager.PFreq.u16ReloadFlag = 1;
			g_GridManager.PFreq.u16WaitReloadFlag = 0;
			TimeWaitcnt =0;			
		}
	}
	else
...
void PFreqPback(void)
{
	...
	if(g_GridManager.PFreq.u16FPLimit >= g_Rated.f32OutputPower)
	{
		...
		g_GridManager.PFreq.u16ReloadFlag = 0;
		g_StateCheck.bit.OverFreqDerating = 0;
		g_GridManager.PFreq.u16PowerFixedPointFlag = 0;	
...
{
	static Uint16 OFreqcnt = 1;

	if(	  (g_CalcResult.Ave.f32FGrid < g_GridManager.PFreq.f32FreqPoint)//50.2
	   	&&(0 == g_GridManager.PFreq.u16ReloadFlag)
	   	&&(0 == g_GridManager.PFreq.u16WaitReloadFlag)		)//50.3Hz  回载结束后才能执行这段
	{
	...
	else if(g_CalcResult.Ave.f32FGrid >= g_GridManager.PFreq.f32FreqPoint)
	{	
		OFreqcnt++;
		if(OFreqcnt >5)
		{
			OFreqcnt = 5;
			g_GridManager.PFreq.u16WaitReloadFlag = 1;
			g_StateCheck.bit.OverFreqDerating = 1;
			if(0 == g_GridManager.PFreq.u16PowerFixedPointFlag)
			{
				g_GridManager.PFreq.u16PowerM = g_CalcResult.Power.f32OutputWatt;
				g_GridManager.PFreq.u16PowerFixedPointFlag = 1;
				g_GridManager.PFreq.u16ReloadFlag = 0;
			}
			g_GridManager.PFreq.u16FPLimit =  g_GridManager.PFreq.u16PowerM  
			* (1 - (g_CalcResult.Ave.f32FGrid - g_GridManager.PFreq.f32FreqPoint) * g_GridManager.PFreq.f32Slop);//???

			if(g_GridManager.PFreq.u16FPLimit < 100)
			{
				g_GridManager.PFreq.u16FPLimit = 100;
			}
		}
	}
	else
	...
	if(1 == g_GridManager.PFreq.u16WaitReloadFlag)
	{
		PFWaitBack();	
	}
	if((g_CalcResult.Ave.f32FGrid > g_GridManager.PFreq.f32FUback)&&(g_CalcResult.Ave.f32FGrid < g_GridManager.PFreq.f32FOback))
	{
		if(1 == g_GridManager.PFreq.u16ReloadFlag)
		{
			PFreqPback();
		}
	}
...
void ConstCosphi(void)
{
	...
	float32 f32TanphiTemp = 0;
	if(g_GridManager.InvQ.f32Cosphi > 1)
	{
		g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit;
		g_GridManager.InvQ.f32Cosphi = 1;
	}
	else if(g_GridManager.InvQ.f32Cosphi < - 1)
	{
		g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit;
		g_GridManager.InvQ.f32Cosphi = 1;
	}
	else if(g_GridManager.InvQ.f32Cosphi > 0.79)
	{
		g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit * g_GridManager.InvQ.f32Cosphi;
	}
	else if(g_GridManager.InvQ.f32Cosphi <- 0.79)
	{
		g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit * (-g_GridManager.InvQ.f32Cosphi);
	...
	f32TanphiTemp = g_GridManager.InvQ.f32Tanphi;
	f32DeltaTanphi = f32Tanphi - f32TanphiTemp;

	if(	f32DeltaTanphi > 0.00483)
	{
		f32TanphiTemp +=0.00483;
	}
	...
	{
		f32TanphiTemp = f32Tanphi;
	}

	if(f32TanphiTemp > 0.75)//cosphi=0.9
	{
		f32TanphiTemp = 0.75;
	}
	else if(f32TanphiTemp < -0.75)
	{
		f32TanphiTemp = -0.75;
	}
	else
	...
	g_GridManager.InvQ.f32Tanphi = f32TanphiTemp;
}
...
void ConstQvar(void)
{
	...
	if(g_GridManager.InvQ.f32Qvar > 0.4845 * g_Rated.f32OutputPower)//cosphi0.9,sin=0.4359
	{
	...
	if(g_GridManager.InvQ.f32Qvar >=0)
	{
		f32Tanphi = g_GridManager.InvQ.f32Qvar;
		if(g_PActiveLimit.f32SMaxLimit > f32Tanphi)
		{
			g_PActiveLimit.f32PReactiveCommand = sqrt(g_PActiveLimit.f32SMaxLimit * g_PActiveLimit.f32SMaxLimit - f32Tanphi * f32Tanphi);
		}
		else
		{
			g_GridManager.InvQ.f32Qvar = g_PActiveLimit.f32SMaxLimit;
			g_PActiveLimit.f32PReactiveCommand = 0.05 * g_Rated.f32OutputPower;
		}
	}
	else
	{
	...
	if(g_PActiveLimit.f32PReactiveCommand < 0.05 * g_Rated.f32OutputPower)
	{
		g_PActiveLimit.f32PReactiveCommand = 0.05 * g_Rated.f32OutputPower;
	}
	if(g_CalcResult.Power.f32OutputWatt > 0.2 * g_Rated.f32OutputPower)
	{
		f32Tanphi = g_GridManager.InvQ.f32Qvar / g_CalcResult.Power.f32OutputWatt;
	}
	else
	{
		f32Tanphi = 5.0 * g_GridManager.InvQ.f32Qvar / g_Rated.f32OutputPower;
	}
//	f32Tanphi = g_GridManager.InvQ.f32Qvar / g_CalcResult.Power.f32OutputWatt;
	f32TanphiTemp = g_GridManager.InvQ.f32Tanphi;
	f32DeltaTanphi = f32Tanphi - f32TanphiTemp;
	if(	f32DeltaTanphi > 0.0243)
	{
		f32TanphiTemp +=0.0243;
	}
	else if(f32DeltaTanphi < -0.0243)
	{
		f32TanphiTemp -=0.0243;
	}
	else
	...
	if(f32TanphiTemp > 2.5)//0.5/0.2
	{
		f32TanphiTemp = 2.5;
	}
	else if(f32TanphiTemp < -2.5)
	{
		f32TanphiTemp = -2.5;
	}
	else
...
void QLineMode(void)
{
	...
	float32 f32Cosphitemp1 = 1;
	float32 f32Cosphitemp2 = 1;
	float32 f32Cosphitemp3 = 1;
	float32 f32Cosphitemp4 = 1;
	float32 f32DeltaTanphi = 0;
	float32 f32TanphiTemp = 0;
...
//	}
	...
	f32Cosphitemp1 = f32Cosphitemp < 0 ? (0 - f32Cosphitemp) : (2.00 - f32Cosphitemp);

	f32Cosphitemp = g_GridManager.InvQ.f32Cosphi2;
	f32Cosphitemp2 = f32Cosphitemp < 0 ? (0 - f32Cosphitemp) : (2.00 - f32Cosphitemp);

	f32Cosphitemp = g_GridManager.InvQ.f32Cosphi3;
	f32Cosphitemp3 = f32Cosphitemp < 0 ? (0 - f32Cosphitemp) : (2.00 - f32Cosphitemp);

	f32Cosphitemp = g_GridManager.InvQ.f32Cosphi4;
	f32Cosphitemp4 = f32Cosphitemp < 0 ? (0 - f32Cosphitemp) : (2.00 - f32Cosphitemp);
	...
	if(1 == g_GridManager.InvQ.u16LockFlag)
	{
		...
		{
			f32Cosphitemp = f32Cosphitemp1;			
		}
		else if(g_CalcResult.Power.f32OutputWatt <= g_GridManager.InvQ.f32Pwatt2)
		{
			f32Cosphitemp = Line(g_CalcResult.Power.f32OutputWatt,g_GridManager.InvQ.f32Pwatt1,f32Cosphitemp1,g_GridManager.InvQ.f32Pwatt2,f32Cosphitemp2);		
		}
		else if(g_CalcResult.Power.f32OutputWatt <= g_GridManager.InvQ.f32Pwatt3)
		{
			f32Cosphitemp = Line(g_CalcResult.Power.f32OutputWatt,g_GridManager.InvQ.f32Pwatt2,f32Cosphitemp2,g_GridManager.InvQ.f32Pwatt3,f32Cosphitemp3);		
		}
		else if(g_CalcResult.Power.f32OutputWatt <= g_GridManager.InvQ.f32Pwatt4)
		{
			f32Cosphitemp = Line(g_CalcResult.Power.f32OutputWatt,g_GridManager.InvQ.f32Pwatt3,f32Cosphitemp3,g_GridManager.InvQ.f32Pwatt4,f32Cosphitemp4);			
		}
		else
		{
			f32Cosphitemp = f32Cosphitemp4;
		}
		if((f32Cosphitemp> 1.21)||(f32Cosphitemp< 0.79))//0.8
		{
			f32Cosphitemp = 1;
		}
		f32Cosphitemp = f32Cosphitemp < 1.00 ? (0 - f32Cosphitemp) : (2.00 - f32Cosphitemp);
		if(f32Cosphitemp > 0)
		{
			g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit * f32Cosphitemp;
		}
		else
		{
			g_PActiveLimit.f32PReactiveCommand = -g_PActiveLimit.f32SMaxLimit * f32Cosphitemp;
...
void InvQvar(void)
{
	...
	switch(g_SaftyConfig.bit.ReactiveMode)
	{
		case 0x0:ConstCosphi();break;
		case 0x1:ConstQvar();break;
		case 0x2:QLineMode();break;
//		case 0x3:QUAmode();break;
//		case 0x4:QUBmode();break;
		default:
		g_GridManager.InvQ.f32Tanphi = 0;
		g_PActiveLimit.f32PReactiveCommand = g_PActiveLimit.f32SMaxLimit;
		break;
...
void OPLoadLimit20ms(void)
{
	...
	if((1 == g_SaftyConfig.bit.FreDeratingEnable)&&(NormalState == g_SysCurrentState))
	{
		PFreqLimit();
	}
	else
	...
	if((1 == g_SaftyConfig.bit.ReactiveEnable)&&(NormalState == g_SysCurrentState))
	{
...
void OILoadLimit20ms(void)
{
	...
	VGridMAX = MIN(g_CalcResult.Rms.f32VGridR , g_CalcResult.Rms.f32VGridS);
	VGridMAX = MIN(VGridMAX , g_CalcResult.Rms.f32VGridT);
