...
{
    EALLOW;
	//SysCtrlRegs.SCSR //need not set scsr.it should be 0x0000
...
void REST_SYS(void)
{
	EALLOW;
	//SysCtrlRegs.SCSR //need not set scsr.it should be 0x0000
    SysCtrlRegs.WDCR= 0x0000;//if the watchdog is enable ,writ 000 to WDCR.wdchk cause
	//immediate device reset or interrupt
    EDIS;
}
//---------------------------------------------------------------------------
// Example: InitPll:
...
// LOSPCP prescale register settings, normally it will be set to default values
   ...
   SysCtrlRegs.LOSPCP.all = 0x0002;	// 低速外设时钟预设因子取默认值010, 即SYSCLK/4

// XCLKOUT to SYSCLKOUT ratio.  By default XCLKOUT = 1/4 SYSCLKOUT
   //SysCtrlRegs.XCLK.bit.XCLKOUTDIV=2;	没有使用xclkout
/*
   	EALLOW;
	SysCtrlRegs.PCLKCR0.bit.ADCENCLK = 1; // Enable ADC peripheral clock
	(*Device_cal)();
...
*/
   ...
   SysCtrlRegs.PCLKCR0.bit.TBCLKSYNC  = 0;     // stop TBCLK within the ePWM
